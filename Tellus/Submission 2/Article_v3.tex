...
The central question motivating this work is whether the tradeoff between carbon assimilation and transit time favors long-term carbon sequestration in regions with high assimilation rates but fast carbon transit times (e.g., tropical ecosystems) or in regions with low assimilation rates but slow transit times (e.g., high-latitude ecosystems). This question arises because tropical forests exhibit large photosynthetic fluxes, yet carbon returns to the atmosphere relatively quickly (1-8 years) \citep{Sierra2021JE,Chanca2025}, whereas high-latitude ecosystems with lower photosynthetic rates retain carbon for much longer ($>$100 years) \citep{Carvalhais2014,Lu2018}. While previous studies have characterized these baseline component fluxes independently, how their dynamic interaction controls the net, cumulative balance of historical carbon storage over decadal timescales remains a critical knowledge gap.
% I think "despite" is wrong, because there is a common underlying reason for lower photosynthetic rates and lower transit times in high-latitude ecosystems.


To address this question, we expand the analyses of \cite{Metzler2024} to global data-driven estimates of net carbon exchange between terrestrial ecosystems in the atmosphere, using bottom-up scaling products (X-BASE) and top-down atmospheric inversions (CarboScope, CAMS). We explore temporal tradeoffs between assimilation and release at the global scale over two decades, between 2001 and 2021. Then, to gain insights into longer timescales, we appl this methodology to process-based simulation output from Earth System Models (CMIP6) and Dynamic Global Vegetation Models (TRENDY v13), spanning the historical period since the beginning of the Industrial Revolution. 
...
% NEP and NBP have never been introduced. For NBP that actually happens in Section 2.1 and for NEP in Section 2.4.
...
\end{equation}
for the $k$ models in the ensemble. 
...
% If we have weights depending on the grid cell, should this sum then not run over $i$ instead of over $s$? Also Eq. (2) strongly sugggests this.
...
Carbon sequestration (CS) is defined here as the accumulated uptake or loss of carbon in terrestrial ecosystems over time, which can be obtained as the integral of carbon storage over time \citep{Sierra2021}. This definition is equivalent to the double integral of the net carbon flux between the atmosphere and the terrestrial biosphere starting at a reference year $t_0$ over a fixed time horizon $T$ \citep{Sierra2021,Metzler2024,Sierra2025}, as
...
% Why does this subsection header include "integrated" and the previous one does not?
...
% Here "Global South" occurs again, which Carlos dislikes in this context, as he mentioned at the first occurrence.
...
% The first sentence does not seem to make sense to me. Something is wrong there. Maybe a full stop is missing after "spread"?
