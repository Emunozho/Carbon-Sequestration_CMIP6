...
def calc_spatial_mean(
    xr_da, lon_name="lon", lat_name="lat", radius=EARTH_RADIUS
):
    """
...
def grid_cell_areas2(xr_da, lon_name="lon", lat_name="lat", radius=EARTH_RADIUS):
    """
    Calculate grid cell areas given 1D arrays of longitudes and latitudes
    for a planet with the given radius.
    ...
    lon_bounds_radian = np.deg2rad(_guess_bounds(lon))
    lat_bounds_radian = np.deg2rad(_guess_bounds(lat))
    area = _quadrant_area(lat_bounds_radian, lon_bounds_radian, radius)
    return area.sum()