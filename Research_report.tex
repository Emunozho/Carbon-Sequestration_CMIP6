...
Carbon Sequestration (CS) obtained as the double integral of NEP from eight CMIP6 models (\textcolor{red}{Supplementary Table~1 (see SI.tex)}) reveal a predominantly positive global distribution, with notable exceptions in polar regions and hyper-arid zones %such as the Sahara, Atacama, Arabian, Central Asia and Gobi deserts 
(Fig.~\ref{fig:CMIP6_NEP_map}a). Forests exhibit the highest CS values, particularly in the Amazon, Eastern and Western North America, the Congo basin, Western Europe, and South East Asia. The latitudinal CS average shown in Fig.~\ref{fig:CMIP6_NEP_map}b demonstrates positive CS across all latitudes, indicating long-term carbon retention globally during the entire 165 year simulation period (1850-2014). Peak CS occurs in tropical latitudes (approximately 25$^{\circ}$S to 20$^{\circ}$N), while mid-latitudes (approximately 20-40$^\circ$N and 20-30$^\circ$S) show the lower values.\footnote{Figure 1 can be improved by moving the legend to the right so it doesn't overlap with the the results for low latitudes. The labels a and b do not need a parenthesis, you can remove them. }
Higher values of CS in tropical and boreal regions in comparison to mid-latitudes suggests that high C assimilation or low C release contribute to retain carbon in the long-term, but the effect of high C assimilation is stronger as suggested by the high CS values in the tropics.
...
\subsection*{Trade-off between high carbon assimilation and slow lagged respiration}
