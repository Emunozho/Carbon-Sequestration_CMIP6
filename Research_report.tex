...
A key metric to understand the temporary nature of the balance between carbon assimilation and release, as well as the lag times between carbon sinks and sources, is the transit time of carbon. This metric, however, is very difficult to determine for the nonlinear and non-equilibrium conditions of the coupled carbon-climate system. Recent studies have shown that, for long periods, it is possible to obtain the cumulative lag effects between assimilation and release, implicitly defined by transit times, without explicitly computing this metric. Instead, an approach that relies on the mathematical integration (double integral) between instantaneous sources and sinks can be employed. This approach, which we define here as integrated Carbon Sequestration (CS), can be applied to the large volumes of available information on net carbon fluxes between the land and the atmosphere derived from both observational products and models.

Our aim in obtaining this metric from existing information is to learn from the historical record on the trade-off between carbon sinks and lagged carbon release. For example, although tropical forests have large photosynthetic fluxes, carbon returns back to the atmosphere relatively quickly. On the contrary, high-latitude ecosystems with lower photosynthetic assimilation rates return carbon to the atmosphere much slowly than tropical ecosystems. Therefore, it is uncertain if the capacity of the terrestrial biosphere to store carbon is mainly controlled by large photosynthetic inputs or slow lagged respiratory fluxes. 
